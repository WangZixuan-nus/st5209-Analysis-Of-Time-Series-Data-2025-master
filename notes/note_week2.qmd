...
aus_arrivals |>
  ...
  ungroup() |>
  autoplot(LaggedArrivals)
#need to sepecift the variable you want to plot
```
...
We can plot them individually, or plot all versions on the same plot.--\> We need to pick a window size which is multiple of season preriods.
...
```{r}
diabetes <- read_rds("D:/AAnus/semester2/ST5209X Analysis Of Time Series Data/_data/cleaned/diabetes.rds")
diabetes |> autoplot()
```
...
The trends become linear. People increase, so the fluctuation increase. ???
...
In the STL model, outliers do not affect the seasonality and trend.
